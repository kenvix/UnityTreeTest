...
public class MtreeComponent : MonoBehaviour
{
    ...
    public Mesh GenerateTree(bool instantAo = false)
    {
        ...
        tree = null;
        ExecuteFunctions(); // 生成树
        tree.Simplify(LODs[LodIndex].simplifyAngleThreshold, LODs[LodIndex].simplifyRadiusThreshold); // 简化枝干
        Mesh mesh = CreateMesh();
        filter.mesh = mesh;
        
        //BakeAo(!instantAo); // 烘培光照Baking Ambient Occlusion
        UpdateMaterials();
        
        // Kenvix's test code
        MTreeExporter.Instance.AddTree(this.name, mesh, tree);
        return mesh;
    }
